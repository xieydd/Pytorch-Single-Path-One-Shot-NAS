...
try:
    ...
    raise ImportError(
        "Please install DALI from https://www.github.com/NVIDIA/DALI to run this data loader")
...
def get_dali_val_loader():
    def gdvl(data_path, batch_size, workers=5, _worker_init_fn=None):
        ...
        pipe.build()
        test_run = pipe.run()
        val_loader = DALIClassificationIterator(pipe, size=int(
            pipe.epoch_size("Reader") / world_size), fill_last_batch=False)
