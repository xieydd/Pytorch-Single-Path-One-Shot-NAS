...
'''
@description: For SPOS SuperNet Config 
'''
...
class SPOSConfig(BaseConfig):
    def build_parser(self):
        ...
        parser.add_argument('--save-iterval', type=int,
                            default=5, help='The frequency of save the model')
...
'''
@description: For SPOS Search(Sample) Config 
'''


class SampleConfig(BaseConfig):
    def build_parser(self):
        parser = get_parser("Sample config")
        parser.add_argument('-a', '--arch', metavar='ARCH', default='Darts',
                            help='model architecture (default: Darts)')
        parser.add_argument('--name', required=True)
        parser.add_argument('--dataset', required=True,
        ...
        parser.add_argument('--topk', type=int, default=5,
                            help='Top k of Sample Supernet')
        parser.add_argument('--search-iters', type=int, default=50000,
    ...
    def __init__(self):
        ...
        super().__init__(**vars(args))
        self.data_path = './data/'
        self.path = os.path.join('sample', self.name)
        self.plot_path = os.path.join(self.path, 'plots')
        self.gpus = parse_gpus(self.gpus)
